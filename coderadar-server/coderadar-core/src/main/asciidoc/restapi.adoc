...
. <<creating-analyzing-job, Add an Analyzing Job>> for your project. Once this is done, coderadar
  will start to analyze all files in the commits defined by the job. Depending on the size of the code base
...
If a project has no file patterns defined, no files will be analyzed.
...
By providing an Analyzing Job to coderadar, you can trigger an analysis of a Project's codebase. Note
the following hints:

* coderadar will automatically start to analyze the codebase once an Analyzing Job has been provided,
unless the Analyzing Job is set to inactive.
* Only one Analyzing Job can be active for a project at a time. You will have to wait until a running job
is finished to start another one.
* New incoming commits to the underlying git repository will automatically be analyzed, unless the Analyzing
Job is set to inactive.
...
include::{snippets}/analyzing-job/create-update/http-request.adoc[]

===== Example Response

include::{snippets}/analyzing-job/create-update/http-response.adoc[]

==== Retrieving an Analyzing Job

===== Example Request

include::{snippets}/analyzing-job/get/http-request.adoc[]

===== Example Response

include::{snippets}/analyzing-job/get/http-response.adoc[]
...
[NOTE]
=====
Metrics will become available as soon as you have provided an <<creating-analyzing-job,Analyzing Job>> for your project.
At that moment, analysis of each commit defined in the job starts. Depending on how many commits that are,
...
[NOTE]
=====
Metric values will become available as soon as you have provided an <<creating-analyzing-job,Analyzing Job>> for your project.
At that moment, analysis of each commit defined in the job starts. Depending on how many commits that are,
analysis will take a while.
=====
...
==== Show all dependencies in a Single Commit
coderadar can analyze a given Java project on its dependencies on its components. This endpoint provides a tree
structure containing nodes. These nodes represent the file structure of the project. Also for every node the
dependencies of this node are listed. Dependencies are analyzed by a the projects base package. For example in coderadar
it is `org.wickedsource.coderadar`.

===== Response Structure

include::{snippets}/analyzer/dependencyTree/response-fields.adoc[]

===== Example Request

include::{snippets}/analyzer/dependencyTree/http-request.adoc[]

===== Example Response

[NOTE]
=====
The response can be rather large depending on the size of you project. The example Response below is limited to the
first 50 lines.
=====
include::{snippets}/analyzer/dependencyTree/http-response.adoc[]

==== Compare the dependencies in in two Commits
coderadar can compare two commits of a project using the changes in the file structure and the projects dependencies.
This endpoint provides a tree structure containing nodes. These nodes represent the file structure of the project and
it changes. Also for every node the dependencies and their changes of this node are listed. Dependencies are analyzed
by a the projects base package. For example in coderadar it is `org.wickedsource.coderadar`.

===== Response Structure

include::{snippets}/analyzer/compareTree/response-fields.adoc[]

===== Example Request

include::{snippets}/analyzer/compareTree/http-request.adoc[]
...
first 50 lines.
=====
include::{snippets}/analyzer/compareTree/http-response.adoc[]
