...
public class MetricsMonitor {
	...
	* @param monitoringIntervalInSeconds the interval in seconds at which to output the current
	*     metric values as a semicolon-separated line. The output format and target can be configured
	*     for the Logger org.wickedsource.coderadar.performance.MetricMonitor in the logging
	*     configuration.
	* @return a Map containing the maximum value for each metric over the whole duration of the
	*     monitoring process.
	*/
	public Map<String, Number> startMonitoring(
			CoderadarRestClient client, int durationInSeconds, int monitoringIntervalInSeconds)
