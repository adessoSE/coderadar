...
public interface GetMetricValuesOfCommitRepository extends Neo4jRepository<CommitEntity, Long> {
  ...
  @Query(
      "MATCH (p:ProjectEntity) WHERE ID(p) = {0} "
          ...
          + "MATCH (p)-[:CONTAINS_COMMIT]->(c:CommitEntity)<-[:CHANGED_IN]-(f:FileEntity) WHERE "
          + "datetime(c.timestamp).epochMillis <= commitTime WITH DISTINCT p, f, commitTime "
          ...
          + "MATCH (p)-[:CONTAINS_COMMIT]->(c:CommitEntity)<-[:VALID_FOR]-(m:MetricValueEntity)<-[:MEASURED_BY]-(f:FileEntity) "
