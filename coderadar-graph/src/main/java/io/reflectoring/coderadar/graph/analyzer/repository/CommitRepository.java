...
public interface CommitRepository extends Neo4jRepository<CommitEntity, Long> {
  @Query("MATCH (p:ProjectEntity)-->(c:CommitEntity) WHERE ID(p) = {0} WITH c SET c.analyzed = false")
  void resetAnalyzedStatus(Long projectId);

  @Query("MATCH (p:ProjectEntity)-->(c:CommitEntity) WHERE ID(p) = {0} RETURN c ORDER BY c.timestamp DESC")
