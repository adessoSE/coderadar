...
public class StopAnalyzingService implements StopAnalyzingPort {
    ...
    @Autowired
    public StopAnalyzingService(GetAnalyzingStatusRepository getAnalyzingStatusRepository, StopAnalyzingRepository stopAnalyzingRepository, GetProjectRepository getProjectRepository) {
    ...
    public void stop(Long projectId) {
        ...
        if (persistedProject.isPresent()) {
            Optional<AnalyzingJob> persistedAnalyzingJob = getAnalyzingStatusRepository.findByProject_Id(projectId);
