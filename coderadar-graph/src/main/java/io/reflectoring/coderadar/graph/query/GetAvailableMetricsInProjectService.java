...
public class GetAvailableMetricsInProjectService implements GetAvailableMetricsInProjectPort {
    ...
    @Autowired
    public GetAvailableMetricsInProjectService(GetProjectRepository getProjectRepository, GetAvailableMetricsInProjectRepository getAvailableMetricsInProjectRepository) {
    ...
    public List<String> get(Long projectId) {
        ...
        if (persistedProject.isPresent()) {
            ...
            throw new ProjectNotFoundException(String.format("A project with the ID '%d' doesn't exists.", projectId));
        }
    }
}
