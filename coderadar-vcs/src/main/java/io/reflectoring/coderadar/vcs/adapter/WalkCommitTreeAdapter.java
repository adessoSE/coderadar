...
public class WalkCommitTreeAdapter implements WalkCommitTreePort {
    @Override
    public void walkCommitTree(String projectRoot, String name, WalkTreeCommandInterface commandInterface) throws UnableToWalkCommitTreeException {
        try {
            ...
            while (treeWalk.next()) {
                if (!treeWalk.getPathString().endsWith(".java")
                        || treeWalk.getPathString().contains("build") || treeWalk.getPathString().contains("out")
                        || treeWalk.getPathString().contains("classes") || treeWalk.getPathString().contains("node_modules")
