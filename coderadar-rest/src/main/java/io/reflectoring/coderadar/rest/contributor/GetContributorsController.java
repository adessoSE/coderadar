...
public class GetContributorsController {
  ...
  @GetMapping(
      ...
      produces = MediaType.APPLICATION_JSON_VALUE)
  public ResponseEntity<List<Contributor>> getContributorsForFile(
      @PathVariable Long projectId, @RequestBody @Validated GetForFilenameCommand command) {
    return new ResponseEntity<>(
        getContributorsUseCase.getContributorsForProjectAndFilename(projectId, command),
        HttpStatus.OK);
  }
