...
class ListAnalyzerConfigurationsFromProjectControllerTest {
  ...
  void returnsTwoGetAnalyzerConfigurationResponsesFromProject() {
    ...
    Assertions.assertEquals(response1.isEnabled(), responseEntity.getBody().get(0).getEnabled());
    ...
    Assertions.assertEquals(response2.isEnabled(), responseEntity.getBody().get(1).getEnabled());
  }
}
