...
public class GitCommitFinder {
    ...
    /**
     * Returns the commit with the given name from the given repository. Returns null, if the commit does not exist.
     *
    ...
    public RevCommit findCommit(Git gitClient, String commitName) {
        try {
            ...
            throw new IllegalStateException(String.format("error accessing git repository at %s", gitClient.getRepository().getDirectory().getAbsolutePath()), e);
        }
    }
