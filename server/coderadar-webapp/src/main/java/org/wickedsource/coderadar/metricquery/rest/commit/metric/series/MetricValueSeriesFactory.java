...
public abstract class MetricValueSeriesFactory<D extends Comparable> {
    ...
    protected abstract List<Commit> getLastCommitPerInterval(Long projectId, LocalDate startDate, LocalDate endDate);

    protected abstract Point<D, Long> createPointForCommit(Commit commit, Long value);

    public DateSeries<D> createSeries(Long projectId, String metricName, LocalDate startLocalDate, LocalDate endLocalDate) {
        DateSeries<D> series = createEmptySeries(startLocalDate, endLocalDate);
        List<Commit> lastCommitsPerDay = getLastCommitPerInterval(projectId, startLocalDate, endLocalDate);
