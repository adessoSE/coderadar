...
public interface ModuleRepository extends CrudRepository<Module, Long> {
    ...
    @Query("select a from CommitToFileAssociation a where substring(a.id.file.filepath, 1, length(:modulePath)) = :modulePath order by a.id.file.filepath")
     ...
     */
    @Query("select m from Module m where m.path = substring(:filepath, 1, length(m.path)) order by m.path")
