...
/**
 * Configures the folder in which Coderadar puts files, including the local clones of VCS
 * repositories.
 */
@Component
  ...
  public List<ParameterValidationError> validate() {
    ...
    try {
      ...
      if (!dir.mkdirs()) {
        return Collections.singletonList(
            new ParameterValidationError(
                String.format("Could not create directory '%s'!", envProperty())));
      }
      return Collections.emptyList();
      ...
      return Collections.singletonList(
          new ParameterValidationError(
              String.format(
                  "Could not access directory '%s' due to %s",
                  envProperty(), e.getClass().getName()),
              e));
    }
  }
