...
body. Since some tools don't allow GET requests with a body, these requests will also be answered
...
will become available as soon as you have created a project and provided valid VCS credentials (see <<Creating a Project>>).
...
== Commit Metrics
You can access the metrics aggregated for each commit using this operation. You have to specify a query object
that defines which metrics you are interested in.

=== Query Structure
include::{snippets}/metrics/commit/request-fields.adoc[]

==== Example Request
This request can be sent using the GET or POST method. See <<usage-of-get, Usage of GET>>.
include::{snippets}/metrics/commit/http-request.adoc[]

==== Example Response
include::{snippets}/metrics/commit/http-response.adoc[]
