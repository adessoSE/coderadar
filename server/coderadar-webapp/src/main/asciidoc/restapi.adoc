...
== Module Metric Values

The following endpoints provide metric value data aggregated by the modules that are currently configured
for you project (see <<chapter-modules, Modules>>).

=== Querying Nested Module Metric Values
This endpoint provides a tree structure containing all modules of the project and their submodules.

==== Query Structure

include::{snippets}/metrics/module/tree/request-fields.adoc[]
...
This request can be sent using either the GET or the POST method. See <<usage-of-get, Usage of GET>>.
====
include::{snippets}/metrics/module/tree/http-request.adoc[]

==== Example Response

include::{snippets}/metrics/module/tree/http-response.adoc[]
