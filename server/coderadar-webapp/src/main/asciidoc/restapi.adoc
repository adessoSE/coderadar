...
The first step is login. To log in in coderadar use <<login, Login>>-API. The user must use the username and the password,
he or she defined.
...
The access token is a Base64 encoded String, that must be added to Authorization header like this:
...
user got as response after login. The refresh token must be sent with request to <<refresh, Token Refresh>>-API.
