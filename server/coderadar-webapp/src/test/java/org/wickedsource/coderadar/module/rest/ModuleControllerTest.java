...
public class ModuleControllerTest extends ControllerTestTemplate {
    ...
    public void createModule() throws Exception {
        ...
        mvc().perform(post("/projects/1/modules")
                .content(toJsonWithoutLinks(resource))
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isCreated())
                .andExpect(containsResource(ModuleResource.class))
                .andDo(document("modules/create",
                        links(halLinks(),
                                linkWithRel("self").description("Link to this module."),
                                linkWithRel("project").description("Link to the project this module belongs to.")),
                        requestFields(
                                fields.withPath("modulePath").description("The path of this module starting at the VCS root. All files below that path are considered to be part of the module."))));
    }
    ...
    public void updateModule() throws Exception {
        ...
        mvc().perform(post("/projects/1/modules/1")
                .content(toJsonWithoutLinks(resource))
                .contentType(MediaType.APPLICATION_JSON))
