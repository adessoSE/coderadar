...
public class ResultMatchers {
    ...
    public static <T> ResultMatcher containsResource(TypeReference<T> typeReference) {
        return result -> {
            ...
            try {
                ...
                throw new RuntimeException(String.format("expected JSON representation of class %s but found '%s'", typeReference, json), e);
            }
        };
    }
    ...
    /**
     * Tests if the response contains an error object with a validation error message for
     * the given field.
     *
