...
/**
 * This test class enables servlet filters for authentication of request. The authentication is disabled in other integration tests.
 */
public class AuthenticationTest extends IntegrationTestTemplate {
    ...
    public void setup() {
        ...
        mvc = MockMvcBuilders.webAppContextSetup(applicationContext).addFilters(springSecurityFilterChain).build();
    }
    ...
    public void getUser() throws Exception {
        ...
        mvc.perform(get("/user/1").header(AuthenticationTokenFilter.TOKEN_HEADER, token)).andExpect(status().isOk());
    }
    ...
    public void getUserForbidden() throws Exception {
        String token = tokenService.generateAccessToken(1L, "radar");
        mvc.perform(get("/user/1").header(AuthenticationTokenFilter.TOKEN_HEADER, token + "invalid")).andExpect(status().isForbidden());
    }
    ...
    public void login() throws Exception {
        ...
        mvc.perform(post("/user/auth")//
                            .content(toJsonWithoutLinks(userLoginResource)).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
