...
public class AnalyzingStrategyControllerTest extends ControllerTestTemplate {
        ...
        .andDo(
            document(
                ...
                links(
                    ...
                    linkWithRel("project")
                        .description(
                            "Link to the project resource this AnalyzingStrategy belongs to.")),
                requestFields(
                    fields
                        .withPath("active")
                        .description(
                            "Defines if this analyzing strategy is active. If it is not active (or there is no strategy for the project at all), no commits of the project will be analyzed. Analyses of commits that are already queued will be finished, however."),
                    fields
                        .withPath("rescan")
                        .description(
                            "Set this to true when updating an AnalyzingStrategy if you want to delete all existing analysis results and restart analysis with the new strategy. You will get an error response if you try rescanning a project while there are running or queued analysis jobs (i.e. when the previous scan over all commits is not yet finished)."),
                    fields
                        .withPath("fromDate")
                        .description(
  ...
  public void getAnalyzingStrategy() throws Exception {
    mvc()
        .perform(get("/projects/1/strategy"))
        .andExpect(status().isOk())
        .andExpect(containsResource(AnalyzingStrategyResource.class))
  ...
  public void getNonExistingAnalyzingStrategy() throws Exception {
    mvc().perform(get("/projects/1/strategy")).andExpect(status().isNotFound());
  }
