package org.wickedsource.coderadar.metric.rest.qualityprofile;
...
public class QualityProfileControllerTest extends ControllerTestTemplate {
    ...
    public void createQualityProfile() throws Exception {
        ...
        mvc().perform(post("/projects/1/qualityprofiles")
                .content(toJsonWithoutLinks(resource))
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isCreated())
                .andExpect(containsResource(QualityProfileResource.class))
                .andDo(document("qualityprofiles/create",
                        links(halLinks(),
                                linkWithRel("self").description("Link to this quality profile."),
                                linkWithRel("project").description("Link to the project this quality profile belongs to.")),
                        requestFields(
                                fields.withPath("profileName").description("The display name of the profile."),
                                fields.withPath("metrics[].metricName").description("Name of the metric that is part of this quality profile."),
                                fields.withPath("metrics[].metricType").description("Either VIOLATION if high metric values should be interpreted as negative or COMPLIANCE if high metric values should be interpreted as positive."))));
    }
    ...
    public void listQualityProfiles() throws Exception {
        mvc().perform(get("/projects/1/qualityprofiles?page=0&size=2"))
    ...
    public void deleteQualityProfile() throws Exception {
        mvc().perform(delete("/projects/1/qualityprofiles/1"))
                .andExpect(status().isOk())
    ...
    @Test
    @DatabaseSetup(SINGLE_PROJECT_WITH_QUALITY_PROFILE)
    @ExpectedDatabase(SINGLE_PROJECT_WITH_QUALITY_PROFILE2)
    public void updateQualityProfile() throws Exception {
        QualityProfileResource resource = profile2();
        mvc().perform(post("/projects/1/qualityprofiles/1")
                .content(toJsonWithoutLinks(resource))
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(containsResource(QualityProfileResource.class))
    ...
    public void getQualityProfile() throws Exception {
        QualityProfileResource resource = profile();
        MvcResult result = mvc().perform(get("/projects/1/qualityprofiles/1")
                .content(toJsonWithoutLinks(resource))
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(containsResource(QualityProfileResource.class))
        ...
        QualityProfileResource profile = fromJson(result.getResponse().getContentAsString(), QualityProfileResource.class);
        ...
        List<String> sentMetrics = resource.getMetrics().stream().map(MetricDTO::getMetricName).sorted().collect(Collectors.toList());
        List<String> returnedMetrics = profile.getMetrics().stream().map(MetricDTO::getMetricName).sorted().collect(Collectors.toList());
