...
public class AnalyzerPluginRegistryTest {
    ...
    public void createsNewPluginInstances(){
        SourceCodeFileAnalyzerPlugin plugin1 = registry.createAnalyzer("org.wickedsource.coderadar.analyzer.domain.DummyAnalyzer");
        SourceCodeFileAnalyzerPlugin plugin2 = registry.createAnalyzer("org.wickedsource.coderadar.analyzer.domain.DummyAnalyzer");
    ...
    public void isAnalyzerRegistered(){
        Assert.assertTrue(registry.isAnalyzerRegistered("org.wickedsource.coderadar.analyzer.domain.DummyAnalyzer"));
        Assert.assertFalse(registry.isAnalyzerRegistered("foo.bar"));
    }

    @Test
    public void configuresPluginsWhenConfigurationFileProvided(){
        SourceCodeFileAnalyzerPlugin plugin1 = registry.createAnalyzer("org.wickedsource.coderadar.analyzer.domain.DummyAnalyzer", new byte[]{'a','b','c'});
        Assert.assertTrue(((DummyAnalyzer) plugin1).isConfigured());
    }

    @Test(expected = IllegalArgumentException.class)
    public void failsOnInvalidConfigurationFile(){
        SourceCodeFileAnalyzerPlugin plugin1 = registry.createAnalyzer("org.wickedsource.coderadar.analyzer.domain.DummyAnalyzer", new byte[]{'c','b','a'});
    }
