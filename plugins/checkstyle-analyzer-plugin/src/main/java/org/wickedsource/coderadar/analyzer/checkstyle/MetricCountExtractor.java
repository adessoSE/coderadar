...
/**
 * This class is an ugly helper class to extract numeric values from checkstyle's AuditEvents. The values are
 * hidden in String messages and are not accessible via the API so this class does some ugly reflection.
 */
public class MetricCountExtractor {

    /**
     * <p>Extracts the metric count from a Checkstyle AuditEvent.</p>
     * <p>An AuditEvent contains a message which describes some coding violation. Usually, each of these
     * messages represents a single violation. Some messages however represent a number of violations. For
     * example the message for CyclomaticComplexityCheck contains a complexity for a method (if it is higher
     * than a configured threshold, otherwise it is not reported).</p>
     *
    ...
    private Long extractLongArgument(AuditEvent event, int position) throws NoSuchFieldException, IllegalAccessException {
